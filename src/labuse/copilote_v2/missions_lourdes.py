...
# FIX-VEILLE (option A) — `preparer_veille` RETIRÉ : c'était le seul producteur de l'`_action` veille,
# mais il n'était appelé NULLE PART (depuis M118, l'intent VEILLE part en `_refus_voie` vers la
# Surveillance). Fonction morte → supprimée avec `_executer_veille`. La pose de veille vit dans les
# zones de veille (`watch_zones`, alertes.py), jamais au chat.
