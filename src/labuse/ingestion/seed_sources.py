...
SOURCES: list[dict] = [
    ...
    dict(name="DVF / valeurs foncières", category="marche", provider="DGFiP / Cerema (DVF+)",
         ...
         technical_notes="Modèle : télécharger + ingérer, requête PAR RAYON (§7bis). Alt. live : Région ODS « demande-de-valeurs-foncierespublic »."),
    ...
    dict(name="Forêts publiques (ONF)", category="environnement", provider="ONF",
         ...
         documentation_url="https://www.data.gouv.fr/fr/datasets/regime-forestier/", endpoint_url=None, legal_notes=None,
         technical_notes="Aucun flux ouvert joignable (PEIGEO/DEAL → HTTP 000 ; absent Région ODS). Fallback : import SHP régime forestier ONF."),
    dict(name="SAR Réunion (PEIGEO)", category="urbanisme", provider="Région Réunion / AGORAH",
         access_type="import", status=S.PARTIEL, reliability_level=R.A_CONFIRMER,
         documentation_url="https://peigeo.re", endpoint_url=None,
         legal_notes="SAR juridiquement SUPÉRIEUR au PLU.",
         technical_notes="PEIGEO injoignable (HTTP 000). Proxy `espacesar` au grain parcelle via Région « potentiel-foncier » ; zonage complet = import SHP SAR."),
    dict(name="Zonage SAFER (DAAF)", category="agricole", provider="DAAF Réunion",
         ...
         technical_notes="Pas de flux ouvert (absent Région ODS) → import SHP « zonage des terres agricoles SAFER » (millésime 2023)."),
    # ── Hubs ──
    ...
    dict(name="OCS GE (IGN)", category="occupation_sol", provider="IGN / Géoplateforme",
         access_type="WFS/téléchargement", status=S.A_FAIRE, reliability_level=R.A_CONFIRMER,
         documentation_url="https://geoservices.ign.fr/ocsge", endpoint_url="https://data.geopf.fr/wfs/ows",
         legal_notes=None, technical_notes="geopf WFS OK mais typename « OCSGE:occupation_du_sol » → 400. Typename exact à confirmer (GetCapabilities). Signal non juridique."),
    ...
    dict(name="ENS (Département)", category="environnement", provider="Département de La Réunion",
         access_type="import", status=S.A_FAIRE, reliability_level=R.A_CONFIRMER,
         documentation_url=None, endpoint_url=None,
         legal_notes="Droit de préemption départemental.", technical_notes="Absent du portail Région ODS. Import Département/PEIGEO requis."),
    ...
    dict(name="DEAL Réunion — trait de côte", category="risques", provider="DEAL Réunion",
         ...
         technical_notes="carto DEAL injoignable (000). Aléa littoral via Géorisques AZI (gaspar/azi ✓) en proxy ; import bandes de recul du trait de côte sinon."),
]
