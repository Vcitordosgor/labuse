...
def test_tuiles_mvt_materialisees_sur_le_run_servi(db_session=None):
    ...
    try:
        with session_scope() as s:
            if not s.execute(text("SELECT to_regclass('mvt_meta')")).scalar():
                pytest.skip("mvt_meta absente (build-mvt pas encore relancé avec l'enregistrement)")
            mvt_label = s.execute(text(
        ...
        pytest.skip(f"base indisponible ({type(exc).__name__}) — cohérence mvt non vérifiable ici")
