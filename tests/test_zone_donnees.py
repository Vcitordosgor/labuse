...
def _fixtures(tmp_path):
    ...
    con.execute(f"""COPY (SELECT * FROM (VALUES
      ('90000000000011','10.71C','O','BOULANGE A','', '97415','01','12','RUE','DE LA GARE','A', DATE '2016-05-04', DATE '2025-12-06'),
      ('90000000000022','10.71C','P','MONSIEUR X','', '97415','NN','5','RUE','DES FLEURS','A', DATE '2001-09-21', DATE '2024-03-01'),
      ('90000000000033','10.71C','O','FERMEE','',      '97415','01','1','RUE','X','F', DATE '1990-01-01', DATE '2019-01-01'),
      ('90000000000044','10.71C','O','HORS 974','',    '75056','01','1','RUE','Y','A', DATE '2010-06-30', DATE '2020-01-01')
            ...
            dateCreationEtablissement, dateDernierTraitementEtablissement)) TO '{stock}' (FORMAT parquet)""")
    con.execute(f"""COPY (SELECT * FROM (VALUES
...
def test_lot1_sirene_jointure_diffusion_position(tmp_path):
    ...
    with session_scope() as s:
        ...
        rows = {x["siret"]: dict(x) for x in s.execute(text(
            ...
            " date_creation, date_dernier_traitement, round(ST_X(geom)::numeric,4) lon, round(ST_Y(geom)::numeric,4) lat "
    ...
    # F2 (OUTILS-4) — date de dernier traitement ingérée (fraîcheur déclarative par établissement)
    assert str(a["date_dernier_traitement"]) == "2025-12-06", "date_dernier_traitement = dateDernierTraitementEtablissement"
