...
export function Loading({ label, className = '', big = false }: { label?: string; className?: string; big?: boolean }) {
  ...
  return (
    <span className={`inline-flex items-center gap-2.5 text-txt-mut ${className}`} role="status" aria-live="polite" aria-busy="true">
      <span className="flex items-center gap-1.5" aria-hidden>
        {[0, 1, 2].map((i) => (
          <span key={i} className={`labuse-dot ${dot} rounded-full bg-mint shadow-[0_0_8px_rgba(92,230,161,0.6)]`}
