...
const PRIX_META: Record<string, { label: string; cls: string }> = {
  ...
  dans_le_marche: { label: 'Dans le marché', cls: 'text-[#E8B44C] border-[#E8B44C]/50 bg-[#E8B44C]/10' },
...
export function ScoreurAdresse({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div data-scoreur-panel className="pointer-events-auto absolute left-1/2 top-16 z-50 w-[420px] max-w-[94vw] -translate-x-1/2 rounded-xl border border-line-2 bg-surface-2 p-4 shadow-2xl">
      <div className="flex items-center justify-between">
        <span className="font-mono text-[10px] tracking-widest text-txt-dim">SCORER UNE ADRESSE — SECONDE OPINION</span>
        <button data-scoreur-close onClick={onClose} className="text-txt-mut hover:text-txt">✕</button>
      </div>
      ...
      <div className="mt-1.5 flex items-center gap-1.5">
        ...
        <button onClick={run} disabled={m.isPending || adresse.trim().length < 3}
          className="shrink-0 rounded-lg bg-mint px-3 py-1.5 text-xs font-medium text-mint-ink hover:brightness-110 disabled:opacity-40">
          {m.isPending ? '…' : 'Scorer'}
        </button>
      </div>
          ...
          {!d.ok ? (
            ...
            <>
              ...
              <p className="mt-0.5 text-[10.5px] text-txt-mut">
                {d.commune} · {d.surface_m2?.toLocaleString('fr-FR')} m² · <span className="font-mono">{d.idu}</span>
              </p>
              {d.score_e?.estimable && (
              ...
              {d.prix && (
                <div className="mt-2 border-t border-line pt-2">
                  ...
                  {d.prix.marge_a_ce_prix_eur != null && (
                    <p className="mt-0.5 text-[11px] text-txt">Marge résiduelle à ce prix : <b>{eur(d.prix.marge_a_ce_prix_eur)}</b></p>
                  )}
              ...
              {d.idu && (
                <button data-scoreur-fiche onClick={() => { select(d.idu!); onClose() }}
                  className="mt-2 text-[11px] font-medium text-mint hover:underline">
