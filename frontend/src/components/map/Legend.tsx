...
export function Legend({ inline = false }: { inline?: boolean }) {
  ...
  const v2 = useV2Actif()
  return (
    <div className={`${inline ? 'rounded-xl bg-surface-2 px-4 py-3' : 'floating absolute bottom-4 right-4 hidden px-4 py-3 sm:block'}`}>
      ...
      {v2 ? (
        <p className="label-caps mb-2">Verdict · Scoring v2</p>
      ) : (
        <Tip block side="top" tip="Classement matrice Q×A (historique) — vocabulaire « dossier », distinct de l'échelle thermique du scoring P servi.">
          <p className="label-caps mb-2">Verdict · Matrice Q×A</p>
        </Tip>
      )}
      ...
      {layers.zonage_parcelle && (
        <div data-legend-zonage>
          <p className="label-caps mb-2 mt-3">Zonage PLU (parcelles)</p>
          <div className="flex flex-col gap-1.5">
