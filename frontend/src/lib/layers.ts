...
export const LAYER_INFO: Record<string, string> = {
  ...
  vefa_neuf:
    'Le prix du logement NEUF, à la maille COMMUNE : la médiane du prix au m² bâti des ventes « en l’état futur d’achèvement » (VEFA) réellement actées, source geo-DVF (DGFiP), sur les 3 dernières années. La commune n’est peinte que si au moins 10 ventes VEFA soutiennent la médiane — sinon elle reste vide (l’absence est dite, jamais un chiffre inventé). Le STOCK et l’écoulement du neuf ne sont PAS servis : ils relèvent de l’enquête ECLN (SDES), conduite en France métropolitaine seulement — hors champ pour La Réunion. Ni prix ni stock ne sont donc extrapolés.',
  equipements:
