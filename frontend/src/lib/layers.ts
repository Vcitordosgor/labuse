...
export const LAYER_INFO: Record<string, string> = {
  ...
  lignes_ht:
    'Les lignes électriques HAUTE TENSION de l’île — le réseau de transport 63 et 90 kV (source : BD TOPO IGN, tension indiquée, lignes aériennes uniquement). C’est une CONTRAINTE potentielle pour un projet (servitudes, reculs) : la servitude exacte n’est pas cartographiée en donnée ouverte et doit être vérifiée auprès du gestionnaire de réseau (EDF SEI). Sur la fiche d’une parcelle, LABUSE indique la distance à la ligne la plus proche. La MOYENNE tension (distribution HTA ~20 kV) n’a pas de couche : au 974 le distributeur est EDF SEI (Enedis n’opère pas dans les DOM) et ses couches ouvertes ont été retirées le 24/12/2025 ; la BD TOPO ne porte que le transport HT. Aucune source ouverte fiable → pas de couche inventée.',
  tcsp:
    'L’axe de transport structurant de l’île : la ligne EXPRESS BAOBAB (réseau Citalis / CINOR), desserte rapide EN SERVICE, corridor Saint-Denis ↔ Sainte-Marie / Sainte-Suzanne. Source : GTFS Citalis (Point d’Accès National, Licence Ouverte). Ce que ça change : à moins de 500 m d’un axe de transport structurant, le règlement de la zone PEUT moduler l’exigence de places de stationnement — c’est un POINT À VÉRIFIER dans le PLU, jamais une réduction promise. Sur la fiche d’une parcelle, LABUSE indique la distance à l’axe et signale le seuil 500 m. (Les autres lignes de bus restent dans « Transport public » ; ici, seul l’axe structurant.)',
  renouv:
