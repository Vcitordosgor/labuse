...
b) **Servies au générique 9 m OPTIMISTE (habitat admis — même gate, effet plus discret)** :
   ...
   phase 4.
c) **Emprise implicite IGNORÉE (3e population — versée par la session B sur décision
   Vic, 28/07)** : **89 zones** où l'article « emprise » dit « non réglementée »
   (`emprise_sol_pct: null` sourcé) alors qu'un % « espace vert/perméable/paysager »
   est imposé — l'emprise bâtie y est bornée à **100 − X** (X = % soustrait TOTAL du
   texte, pas le sous-minimum de pleine terre : cas « X % perméable dont Y % pleine
   terre » du Tampon/Saint-Paul). Le moteur les traite aujourd'hui comme non
   contraintes en emprise (bornage par reculs+hypothèses seulement) → optimiste par le
   même mécanisme de fond. **Table complète zone par zone (commune, % soustrait,
   emprise implicite) : `PLU_NUIT_ANALYSES_MATIN_B.md` §D.** Même exigence que a) et
   b) : mesure d'impact + question des tiers servis AVANT toute implémentation
   (décision produit explicitement réservée par Vic).
d) Hors périmètre : Saint-Paul (mode strict, gate inactif) ; zones hauteur_mode=prospect.
...
5. Emprise implicite (§5.c, 89 zones) : mesure d'impact dédiée puis arbitrage produit
   Vic — implémentation séparée du correctif du gate (elle DURCIT la capacité sur les
   89, là où le gate ne fait que rendre l'interdit exact).
