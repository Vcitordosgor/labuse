...
- [ ] **N°1 (Vic 04/08, verdict 14/14 bâties) — RECHARGEMENT DE LA COUCHE BATIMENT depuis la source à jour.** La couche est aveugle sur les têtes : 14/14 brûlantes suspectes bâties (retirées), 768/1 143 têtes servies (67 %) à emprise ≈ 0 SANS indice (jamais suspectables) — taux d'erreur réel mesuré sur échantillon de 100 (cf. TRAIN1 rapport). 1 061 parcelles piscine-sur-couche-vide, 456 secteurs, 24 communes → recharger par commune (Saint-Paul d'abord). Conditionne : levée des 15 exceptions (CH1893 + les 14, motifs en base), arbitrage des 32 restantes, filtre client bâti. Si le taux échantillon est élevé : BLOQUANT PRODUIT, plus une dette.
