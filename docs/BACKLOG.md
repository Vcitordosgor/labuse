...
- [ ] **[S] Vic — bascule run servi INCOMPLÈTE (révélé M31)** : tuiles = `q_v8_calibre`, constantes code (`Q_A_RUN_LABEL` + `api.ts` SOURCE + bundle) = `q_v7_defisc`. Aucun env n'aligne les 3. Terminer = avancer les 2 défauts + `npm run build` + `build-mvt` en un geste. Risque prod : sans `LABUSE_SERVED_RUN`, sert le run pré-M28. Un rouge de test consigné (test_run_serving_coherence). §PC1.
