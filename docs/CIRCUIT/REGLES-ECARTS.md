...
|---|---|---|---|---|---|---|
| E1 | drapeau `sous_800m` (fiche `distance_arret_m`) | `proche = d <= 800` (api/app.py) | L151-36 (loi 2025-1129, vigueur 28/11/2025) : « situées à **moins de** huit cents mètres » | seuil comparé LARGEMENT au lieu de STRICTEMENT | `d < 800` — **corrigé au lot 6** (catégorie arithmétique pure du mandat) | d = 800 m exactement (distance entière) servait « sous 800 m » à tort ; aucun autre cas |
