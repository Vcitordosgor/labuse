...
> **Pourquoi on garde le verdict « opportunité » tout en ajoutant le badge.** Ces parcelles ne sont pas
> fausses, mais leur intérêt promoteur dépend fortement de l'assemblage, de la micro-opération ou du
> contexte local. Le badge **nuance** la lecture sans **punir** le signal : le déclassement métier
> (`SURFACE_MIN_M2 = 250`) reste seul juge du verdict, et il a déjà écarté tout ce qui est sous 250 m².

### Option A — APPLIQUÉE (affichage seulement, aucun changement de verdict) ⭐
**Tag « micro-opportunité » sur les opportunités ≤ 500 m²** (badge d'affichage, basé sur la **surface**,
...
démo : on assume la petite taille au lieu de la masquer → **crédibilité**.
**Implémentation (présentation pure, 0 scoring touché)** : drapeau dérivé `verdict.micro_opportunite`
(`resume.is_micro_opportunite`, seuil `MICRO_OPPORTUNITE_MAX_M2 = 500`), affiché dans la **fiche**
(badge + microcopy + mise en avant assemblage), la **liste** et la **shortlist** (tag « micro »), et
l'**export PDF/Markdown**. Verrouillé par test (`tests/test_micro_opportunite.py`).
...
1. ✅ **Option A appliquée** — badge « micro-opportunité » ≤ 500 m², **sans changer aucun verdict**
   (les 156 restent « opportunité »), sans toucher aux données ni au scoring.
2. ⏳ **Option B en attente** — déclassement économique (charge foncière ≤ 0 / SDP < 50 m²) : **change
   des verdicts** → à instruire séparément, après validation de Vic.

---

*Audit + Option A (affichage). 0 verdict modifié, 0 donnée métier touchée, 0 cascade relancée, 0 autre
commune. Chiffres reproductibles via `scripts`/SQL sur la base Saint-Paul au 2026-06-20.*
