...
- Fonds de carte (B1, décision Vic 24/08) : on garde les tuiles CARTO anonymes pour l'instant, risque assumé. Si CARTO throttle ou coupe (fond « Sombre » = défaut de tous les clients), les issues sont : compte CARTO + clé, bascule du défaut sur Plan IGN, ou style auto-hébergé.
- EN ATTENTE (post-passe de check) : re-run cascade pour servir age_dirigeant=0 → `labuse dryrun-evaluate --label q_v10_m129 --no-resume` (~117 min, les 24 communes ; --no-resume impératif). Puis `labuse build-mvt`. Tant qu'il n'est pas joué, l'app sert l'ancien opportunity_score. score-v2 non nécessaire (le tier ne dépend pas de l'âge). À grouper avec d'éventuels autres changements de cascade issus des blocs CRM/veille/projets/sources.
- Table `veilles` en EXTINCTION : plus aucun chemin de création vivant (endpoints = liste/suppression/évaluation cron). Le système de veille vivant est `watch_zones` (Surveillance), 4 déclencheurs réels (dvf/permis/bodacc/zonage). Colonnes criteres/frequence inertes, non droppées. 7 veilles permis existantes toujours évaluées.
- À vérifier : 4 erreurs pré-existantes dans test_courrier (SyntaxError SQL au setup de la fixture) — sans rapport avec la veille, mais la suite courrier ne teste donc rien aujourd'hui. À réparer.
- LICENCES À CONFIRMER (S5, décision Vic) : 8 sources sans licence vérifiée alimentent un produit commercial. Priorité haute — Potentiel foncier Région id 9 (proxy SAR, cascade sur 431 663 parcelles), RPG IGN/ASP (signal agricole), Parc National INPN (HARD_EXCLUDE cascade). Priorité moindre — data.regionreunion id 15, INPN/patrinat, Office de l'eau, VRD/SPANC, PLH des 5 EPCI. Détail dans le compte-rendu FIX-SOURCES.
- Copilote : batterie de 20 questions à 20/20 (25/08). Règle acquise — « un outil = une valeur menée » (compter_permis ≠ delais_instruction). Dernier outil multi-valeurs non traité : stats_commune (SRU + logements + propriétaires) — à surveiller. Q3/Q13 prouvés en déterministe seulement (throttle provider au run réel).
- DA mauve : VÉRIFIÉ À L'ÉCRAN par Vic (25/08) — le mauve n'apparaît que sur les surfaces IA. Rien à corriger, pas de mandat DA. Reste seulement à resserrer le commentaire LOI-0 du tailwind (« violet = premium/IA/outils » → « violet = surfaces IA uniquement ») lors d'un prochain passage sur ce fichier, pour qu'aucune session future ne s'en autorise ailleurs.
- GRAND BALAYAGE CLOS (25-26/08) : 3 cycles, ~100 missions/questions, findings GB-001→014 tous soldés (dont GB-011 🔴 heal de migrations menteur — réparé + splitter SQL partagé ; GB-013 idempotence Courrier ; GB-014 multi-intentions/clarification/créole). 6 gardées vertes sur 3 cycles. Passe blanche inconditionnelle au cycle 3 (grand oral Copilote 40/40, LLM réel). Dette maintenue : IDOR à deux comptes avant le 2e client ; rapports dans docs/audit-2026-08/GRAND-BALAYAGE/.
- CAMPAGNE GRAND BALAYAGE DÉFINITIVEMENT CLOSE (26/08) : 5 cycles, ~800 missions/passes (dont 500 seedées au cycle 5), findings GB-001→040 — tous soldés sauf 3 🟡 en dette assumée : GB-037 (œ→oe courrier PDF, intendé), GB-038 (comptage EN défléchi), GB-040 (NOTE DE DÉPLOIEMENT : uvicorn --workers N en prod, mono-worker = p95>3s sous 50 connexions). Verdict d'ensemble : zéro faux chiffre servi sur 200 parcelles aléatoires + 50 questions Copilote spot-checkées, zéro fuite, zéro régression inter-cycles. Filet permanent : gardées G1-G6 + batteries Copilote + corpus fuzz lot-v.csv (rejouables après tout gros chantier). Rapports : docs/audit-2026-08/GRAND-BALAYAGE/.
