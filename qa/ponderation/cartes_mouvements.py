...
Pour chaque parcelle dont le tier change entre q_v9_pond_avant et q_v9_pond_apres avec la tête
...
de pondération, seuil de zone. Sortie : qa/ponderation/cartes_mouvements.html. Lecture seule.
"""
...
OUT = os.path.join(os.path.dirname(__file__), "cartes_mouvements.html")
...
html = (f"<!doctype html><meta charset='utf-8'><title>Pondération — mouvements en tête</title>"
        ...
        f"<span class='muted'>(mesure à blanc q_v9_pond_avant → q_v9_pond_apres, ortho IGN)</span></h1>"
        + "".join(cards))
