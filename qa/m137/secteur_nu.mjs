...
const info = await page.evaluate(() => {
  ...
  const sect = stats.find(s => /NU/.test(s.querySelector('.stat-l')?.textContent || ''));
  return {
...
await soft(async () => {
  const i = page.locator('[aria-label="Méthode : Nu"]');
...
await b.close();
const ok = /^NU/.test(info.secteurLabel || '') && info.aUnI && /€\/m²/.test(info.secteurValeur || '');
