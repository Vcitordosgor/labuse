...
await page.waitForTimeout(8000); // laisser les DEUX orthos peindre
await page.screenshot({ path: `${OUT}/tm_fiche.png` });
// crop sur le comparateur (zone centrale)
await soft(async () => { const bx = await page.locator('.select-none').first().boundingBox(); if (bx) await page.screenshot({ path: `${OUT}/tm_fiche_zoom.png`, clip: { x: bx.x, y: bx.y, width: bx.width, height: bx.height } }); }, 'crop');
await b.close();
console.log('fiche done');
