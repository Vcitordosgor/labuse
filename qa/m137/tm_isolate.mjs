...
// the two map divs: left=index0, right=index1 (the clipped one)
const divs = await page.evaluate(() => {
  const wrap = document.querySelector('.select-none');
  const kids = [...(wrap?.children||[])].filter(c => c.querySelector && c.querySelector('canvas'));
  return kids.length;
});
console.log('map divs w/ canvas:', divs);
// Show ONLY the RIGHT (now) map: full clip off + bring on top
await page.evaluate(() => {
  const wrap = document.querySelector('.select-none');
  const mapDivs = [...wrap.children].filter(c => c.querySelector && c.querySelector('canvas'));
  // right map is the one with clipPath set
  const right = mapDivs.find(d => d.style.clipPath && d.style.clipPath !== 'none') || mapDivs[1];
  right.style.clipPath = 'none';
  right.style.zIndex = '5';
});
await page.waitForTimeout(3500);
await page.screenshot({ path: `${OUT}/tm_right_only.png` });
// Now HIDE the right map entirely → see the LEFT (past) map
await page.evaluate(() => {
  const wrap = document.querySelector('.select-none');
  const mapDivs = [...wrap.children].filter(c => c.querySelector && c.querySelector('canvas'));
  const right = mapDivs[1];
  right.style.display = 'none';
});
await page.waitForTimeout(2500);
await page.screenshot({ path: `${OUT}/tm_left_only.png` });
await b.close();
console.log('done');
