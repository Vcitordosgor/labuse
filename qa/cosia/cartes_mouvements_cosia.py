...
Pour chaque parcelle dont le tier change entre q_v8_calibre et q_v10_cosia_apres avec la tête
...
de pondération, seuil de zone. Sortie : qa/ponderation/cartes_mouvements_cosia.html. Lecture seule.
"""
...
OUT = os.path.join(os.path.dirname(__file__), "cartes_mouvements_cosia.html")
...
JOIN parcel_p_score_v2 b ON b.parcelle_id=a.parcelle_id AND b.run_id='q_v10_cosia_apres'
...
html = (f"<!doctype html><meta charset='utf-8'><title>Pondération — mouvements en tête</title>"
        ...
        f"<span class='muted'>(mesure à blanc q_v8_calibre → q_v10_cosia_apres, ortho IGN)</span></h1>"
        + "".join(cards))
